...
async def process_quiz_answer(callback: CallbackQuery, state: FSMContext):
    ...
    # Задержка 3 секунды, чтобы пользователь успел прочитать объяснение
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    res_text = (
        ...
        f"Получено билетов за эту попытку: <b>{total_for_attempt}</b>\n"
    )

    if bonus_tickets:
        nums_str = ", ".join([f"№{n:05d}" for n in bonus_tickets])
        res_text += f"🎫 Твои бонусные билеты: {nums_str}\n"
    else:
        if score < 8:
            res_text += "К сожалению, этого недостаточно для бонусных билетов. Попробуй ещё раз!"
