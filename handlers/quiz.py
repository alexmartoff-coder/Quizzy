...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    from database.db import issue_ticket, update_ticket_result

    # Обновляем основной билет
    ...
    for _ in range(bonus_count):
        ...
        if bn:
            await update_ticket_result(bn, "completed", score)
            bonus_ticket_nums.append(bn)

    total_tickets = 1 + bonus_count
    all_tickets_str = ", ".join([f"№{t:05d}" for t in ([t_num] + bonus_ticket_nums)])
