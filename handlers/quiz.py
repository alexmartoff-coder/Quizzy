...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        ...
        if bt:
            await update_ticket_result(bt, "active", 0) # Бонусные билеты сразу активны
            bonus_tickets.append(f"№{bt:05d}")

    await update_ticket_result(t_num, "active", score)
    await finish_quiz_session(user_id)
    await state.clear()

    tickets_str = f"№{t_num:05d}"
    if bonus_tickets:
    ...
    msg = (
        ...
        f"🎟 <b>Вы получили билетов: {1 + bonus_count} шт.</b>\n"
        f"Номера билетов: {tickets_str}\n\n"
        "Они уже участвуют в розыгрыше!"
    )
