...
async def cmd_rules(message: Message):
    rules_text = (
        ...
        "1. **Участие** — платное. Стоимость одной попытки — **99 ₽**.\n"
        "2. **За каждую оплату** вы получаете:\n"
        ...
        "     - 8/10 → +1 билет\n"
        "     - менее 8 → бонусов нет\n"
        "3. **Все билеты** (базовые + бонусные) участвуют в розыгрыше.\n"
        "4. **Сбор билетов** автоматически прекращается, как только набрано **2500 билетов**.\n"
        "5. **После остановки сбора**:\n"
        "   - ❌ играть и оплачивать нельзя\n"
        "   - ✅ можно посмотреть свои билеты, лидерборд и правила\n"
        "6. **Розыгрыш** проводится честно через генератор случайных чисел **[random.org](https://www.random.org)**.\n"
        "7. **Прямой эфир** с определением победителя состоится в канале **@mozgo_boy** — дата и время будут объявлены там же.\n"
        "8. **Один участник** может купить неограниченное количество попыток — чем больше билетов, тем выше шанс выиграть.\n\n"
