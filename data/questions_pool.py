...
QUESTIONS_POOL = [
    ...
    {"id": 11, "question": "В какой стране был продан первый официальный iPhone?", "options": ["США", "Великобритания", "Китай", "Германия"], "correct_index": 0, "explanation": "Продажи первого iPhone начались в США 29 июня 2007 года."},
    ...
    # Добавляем еще много вопросов разных категорий для демонстрации пула...
]

# Генерируем дополнительные вопросы программно для объема (в реальности здесь были бы уникальные данные)
for i in range(50, 150):
    ...
    QUESTIONS_POOL.append({
        ...
        "correct_index": random.randint(0, 3) if 'random' in globals() else (i % 4),
        "explanation": f"Это объяснение для вопроса №{i}. Всегда полезно знать детали!"
    })

import random
# Перемешиваем пул один раз при импорте, если нужно, но лучше выбирать случайно при старте квиза.
